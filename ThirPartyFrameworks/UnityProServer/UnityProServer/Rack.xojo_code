...
Inherits COM.IDispatch
	...
	#tag Method, Flags = &h0
		Sub Constructor(connectEvents As Boolean = False)
		    ...
		    riid = COM.IIDFromString( "{952F5893-8F98-49E2-BFBD-A5F787D2BF7D}" )
		    clsid = COM.IIDFromString( "{B8FF426A-5F37-4B83-9642-4C08587CF15F}" )
	...
	#tag Method, Flags = &h0
		Shared Function IID() As MemoryBlock
		  #if TargetWin32
		    Return COM.IIDFromString("{952F5893-8F98-49E2-BFBD-A5F787D2BF7D}")
		  #endif
		End Function
	#tag EndMethod
	...
	#tag Method, Flags = &h0
		Function IsExtended() As Integer
		  #if TargetWin32
		    If mThis = Nil Then Raise New NilObjectException
		    Dim func As New IsExtended_Func1(mThis.Ptr( 0 ).Ptr(9 * COM.SIZEOF_PTR ))
		    Dim resultCode As Integer
		    Dim Return_pExtended_Param As Integer
		    resultCode = func.Invoke(mThis, Return_pExtended_Param)
		    If resultCode = 0 Then
		      Return Return_pExtended_Param
		    Else // Throw Exception
	...
	#tag DelegateDeclaration, Flags = &h1
		Protected Delegate Function IsExtended_Func1(this As Ptr, ByRef pExtended_Param As Integer) As Integer
	#tag EndDelegateDeclaration
	...
	#tag Method, Flags = &h0
		Function IsMain() As Integer
		  #if TargetWin32
		    If mThis = Nil Then Raise New NilObjectException
		    Dim func As New IsMain_Func1(mThis.Ptr( 0 ).Ptr(14 * COM.SIZEOF_PTR ))
		    Dim resultCode As Integer
	...
	#tag DelegateDeclaration, Flags = &h1
		Protected Delegate Function Modules_Get_Func1(this As Ptr, ByRef ppModules_Param As Ptr) As Integer
	#tag EndDelegateDeclaration
	...
	#tag Method, Flags = &h0
		Sub Operator_Convert(rhs As COM.IUnknown)
		    ...
		    Dim p As Ptr
		    If 0 = rhs.QueryInterface( UnityProServer.IRack.IID, p ) Then
		      mThis = p
		    Else
	...
	#tag ComputedProperty, Flags = &h0
		#tag Getter
			Get
			  #if TargetWindows
			    If mThis = Nil Then Raise New NilObjectException
			    Dim ppModules_Param As Ptr
			    Dim func As New Modules_Get_Func1( mThis.Ptr( 0 ).Ptr( 18 * COM.SIZEOF_PTR ) )
			    Dim resultCode As Integer = func.Invoke( mThis, ppModules_Param )
			    If 0 = resultCode Then
			      If Nil <> ppModules_Param Then
			        Return New UnityProServer.IModules( ppModules_Param )
			      End If
			    Else
			      Raise New COM.COMException("Failed on Modules", resultCode )
			    End If
			  #endif
			  
			End Get
		#tag EndGetter
		Modules As UnityProServer.IModules
	#tag EndComputedProperty
	...
	#tag ComputedProperty, Flags = &h0
		#tag Getter
			Get
			    ...
			    Dim func As New PartNumber_Get_Func1( mThis.Ptr( 0 ).Ptr( 13 * COM.SIZEOF_PTR ) )
	...
	#tag ComputedProperty, Flags = &h0
		#tag Getter
			Get
			    ...
			    Dim func As New TopoAddress_Get_Func1( mThis.Ptr( 0 ).Ptr( 11 * COM.SIZEOF_PTR ) )
	...
	#tag ComputedProperty, Flags = &h0
		#tag Getter
			Get
			    ...
			    Dim func As New Version_Get_Func1( mThis.Ptr( 0 ).Ptr( 12 * COM.SIZEOF_PTR ) )
