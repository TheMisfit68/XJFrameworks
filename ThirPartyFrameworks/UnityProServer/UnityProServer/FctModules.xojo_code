...
Inherits COM.IDispatch
	...
	#tag Method, Flags = &h0
		Sub Constructor(connectEvents As Boolean = False)
		    ...
		    riid = COM.IIDFromString( "{554EAE52-4F5B-11D5-88F1-00062926BB4F}" )
		    clsid = COM.IIDFromString( "{554EAE51-4F5B-11D5-88F1-00062926BB4F}" )
	...
	#tag Method, Flags = &h0
		Sub Export(FileNameToExport_Param As String, ExportOption_Param As Integer)
		  #if TargetWin32
		    If mThis = Nil Then Raise New NilObjectException
		    Dim func As New Export_Func2(mThis.Ptr( 0 ).Ptr(12 * COM.SIZEOF_PTR ))
		    Dim resultCode As Integer
	...
	#tag Method, Flags = &h0
		Sub ImportWithStrategy(FileNameToImport_Param As String, psrImportStrategy_Param As epsrImportOption)
		  #if TargetWin32
		    If mThis = Nil Then Raise New NilObjectException
		    Dim func As New ImportWithStrategy_Func2(mThis.Ptr( 0 ).Ptr(14 * COM.SIZEOF_PTR ))
		    Dim resultCode As Integer
		    Dim Local_FileNameToImport_Param As Ptr
		    Local_FileNameToImport_Param = COM.SysAllocString( FileNameToImport_Param )
	...
	#tag Method, Flags = &h0
		Function Item(FctModuleName_Param As String, Id_Param As Integer) As UnityProServer.IFctModule
		  #if TargetWin32
		    If mThis = Nil Then Raise New NilObjectException
	...
	#tag Method, Flags = &h0
		Sub Move(IndexKeyFrom_Param As Integer, IndexKeyTo_Param As Integer)
		  #if TargetWin32
		    If mThis = Nil Then Raise New NilObjectException
		    Dim func As New Move_Func2(mThis.Ptr( 0 ).Ptr(15 * COM.SIZEOF_PTR ))
		    Dim resultCode As Integer
	...
	#tag ComputedProperty, Flags = &h0
		#tag Getter
			Get
			    ...
			    Dim func As New _NewEnum_Get_Func1( mThis.Ptr( 0 ).Ptr( 13 * COM.SIZEOF_PTR ) )
