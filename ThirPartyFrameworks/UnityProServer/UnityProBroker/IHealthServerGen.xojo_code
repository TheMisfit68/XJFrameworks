...
Inherits COM.IDispatch
	...
	#tag Method, Flags = &h0
		Function GetCountForRelease() As Integer
		    ...
		    Dim func As New GetCountForRelease_Func1(mThis.Ptr( 0 ).Ptr(11 * COM.SIZEOF_PTR ))
	...
	#tag Method, Flags = &h0
		Function GetMultipleInstanceCount() As Integer
		  #if TargetWin32
		    If mThis = Nil Then Raise New NilObjectException
		    Dim func As New GetMultipleInstanceCount_Func1(mThis.Ptr( 0 ).Ptr(9 * COM.SIZEOF_PTR ))
		    Dim resultCode As Integer
	...
	#tag Method, Flags = &h0
		Function GetMultipleInstanceInterfaces(ByRef piCount_Param As Integer) As Variant
		  #if TargetWin32
		    If mThis = Nil Then Raise New NilObjectException
		    Dim func As New GetMultipleInstanceInterfaces_Func2(mThis.Ptr( 0 ).Ptr(8 * COM.SIZEOF_PTR ))
		    Dim resultCode As Integer
	...
	#tag Method, Flags = &h0
		Sub GetServerNames(ByRef pbstrProductName_Param As String, ByRef pbstrFileName_Param As String)
		  #if TargetWin32
		    If mThis = Nil Then Raise New NilObjectException
		    Dim func As New GetServerNames_Func2(mThis.Ptr( 0 ).Ptr(10 * COM.SIZEOF_PTR ))
		    Dim resultCode As Integer
	...
	#tag Method, Flags = &h0
		Function IsAlive() As Int16
		  #if TargetWin32
		    If mThis = Nil Then Raise New NilObjectException
		    Dim func As New IsAlive_Func1(mThis.Ptr( 0 ).Ptr(7 * COM.SIZEOF_PTR ))
		    Dim resultCode As Integer
